...
1. Browse Google Earth Engine's data catalog
...
1. What is the spatial resolution of this band?
...
In the popped out window of the time series chart, download the comma-seperated value (CSV) text file by clicking the <span style="color: black;background-color: #F1F1F1;">Download CSV</span> button. I save my file into  `data/no2_delhi_ee-chart.csv` of this Github repository.

Notice in the downloaded CSV that all the values are 0, 0.001 or missing. Apparently the download functionality rounds values to the nearest 0.001. So to get the kind of precision we need let's multiply each image in the collection by 1000 (going from moles to micromoles of NO~2~ per m^2^) using the `map()` function, being sure to reapply the datetime stamp for the image:
...
Hit Run and download the CSV again from the chart.
